...
const InvoiceCard: React.FC<InvoiceCardProps> = ({ title, invoices }) => {
  ...
  return (
    <div className="bg-white p-4 shadow rounded flex-1">
      {/* Card header with title and invoice count */}
      <h2 className="text-base font-bold mb-2">
        {title} ({invoiceCount})
      </h2>
      <ul className="min-h-[200px] space-y-1">
        {invoiceCount === 0 ? (
          <li className="py-1 text-center text-sm text-gray-600">
          ...
          <>
            {currentInvoices.map((inv) => {
              const partnerName =
                inv.invoiceType === 'Faktur masuk'
                  ...
                  : 'N/A';
              return (
                <li key={inv._id} className="flex items-center py-1">
            ...
            {placeholders.map((_, index) => (
              <li key={`placeholder-${index}`} className="flex items-center py-1">
                <div className="w-full flex justify-between items-center">
                  <span>&nbsp;</span>
                  <span>&nbsp;</span>
                </div>
      ...
      <div className="mt-1 h-8 flex justify-between items-center">
        <button
...
const HomeScreen = () => {
  ...
  // Filter invoices by status for each card.
